...
void readVoltageandSOC(float *voltage, float *soc) {
   ...
   // inicializace I2C
    ...
    ESP_LOGI(TAG, "i2c install driver");
    
    esp_err_t res = i2c_driver_install(I2C_NUM_0, I2C_MODE_MASTER, 0, 0, 0);
	if (res == ESP_FAIL) {
    	ESP_LOGW("I2C", "Ovladač již je nainstalován nebo došlo k chybě.");
    	// Zde můžete pokračovat, ovladač už běží
	} else if (res != ESP_OK) {
    	ESP_LOGE("I2C", "Instalace selhala: %s", esp_err_to_name(res));
	}
    ...
    if (max17048_get_voltage(voltage) == ESP_OK &&
        ...
        ESP_LOGI(TAG, "Battery: %.3f V, %.1f %%", *voltage, *soc);
        ESP_LOGI(TAG, "Battery: %f , %f", *voltage, *soc);
    } else {
		ESP_LOGI(TAG, "Error reading battery.");
	}
#else
  ESP_LOGI(TAG, "Set fake battery status");
  *voltage = 3.7;
  *soc = 95;    
#endif    
}
