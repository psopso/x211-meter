...
void send_data(const char *status, const char *reason, time_t startTime, bool first_boot, char *serialno, bool status_only);
// ==============================================================================
// HLAVNÍ BĚH PROGRAMU
// ==============================================================================
void app_main(void) {
...
//	ESP_LOGI(TAG, "Target: %s", IDF_TARGET)
    ...
    ESP_ERROR_CHECK(ret);
        ...
        ESP_LOGI(TAG, "Power Enable: %d", POWER_ENABLE_GPIO);

        //Převodník UART - RS485 3,3 V - ARK / RJ11 - Waveshare 4777
        //Zřejmě nastavení RSE na HIGH - příjem.
        //gpio_set_direction(RS485_ENABLE_RX_PIN, GPIO_MODE_OUTPUT);
        //gpio_set_level(RS485_ENABLE_RX_PIN, 0);
        //ESP_LOGI(TAG, "RS485_RX_ENABLE Enable: %d", RS485_ENABLE_RX_PIN);
    #endif
...
//mqtt                custom_mqtt_send_status(CMD_STATUS, "FAILED", "NTP failed", bootCount, wakeUpCycleCounter, firstBootTime, 0, 0, 0, check_signal_strength(), 0, "", "", 0);
				send_data("OK", "First boot", startTime, first_boot, "0", true);
    ...
    while (!rs485completed) {    
        ...
        if (length > 0) {
            ...
            if (dlms_parse_frame(buffer, length, &parsed_data) == ESP_OK) {
                ...
                strcpy(serialno, parsed_data.obis_96_1_1);
...
//mqtt            custom_mqtt_send_status(CMD_STATUS, "FAILED", "rs485 failed", bootCount, wakeUpCycleCounter, firstBootTime, 0, 0, 0, check_signal_strength(), 0, "", "", 0);
			send_data("FAILED", "rs485 failed", startTime, first_boot, "0", true);
            esp_sleep_enable_timer_wakeup((uint64_t)DEEP_SLEEP_INTERVAL_MIN_NTP * 60 * 1000000);
            gpio_set_level(POWER_ENABLE_GPIO, 0); 
	    ESP_LOGI(TAG, "Going into deep sleep mode after timeout for %llu", (unsigned long long)DEEP_SLEEP_INTERVAL_MIN_NTP);
            esp_deep_sleep_start();                
        }
    }
...
//mqtt        custom_mqtt_send_data();
		if (first_boot)
			send_data("OK", "First boot", startTime, first_boot, serialno, false);
		else
			send_data("OK", "After wakeup", startTime, first_boot, serialno, false);
...
void send_data(const char *status, const char *reason, time_t startTime, bool first_boot, char *serialno, bool status_only) {
	ESP_LOGI(TAG, "Odesílám data a status: Status: %s, first_boot %d, status_only: %d", status, first_boot, status_only);
...
//    strncpy(out_data.next_wakeup_str, next_wakeup_str_pers, sizeof(out_data.next_wakeup_str) - 1);
    float voltage, soc;
    battery_get_status(&voltage, &soc);
    ...
    out_data.rssi = check_signal_strength(); // Bude definováno jinde, nebo přesunuto do comm
    out_data.battery_voltage = voltage;
    out_data.battery_soc = soc;
    strcpy(out_data.serialno, serialno);
    ...
    if (comm_is_connected()) {
		...
		if (true) {
			...
			//comm_send(MSG_TYPE_BATTERY, &out_data);
		}
		if (!status_only) {
...
//mqtt        custom_mqtt_send_status(CMD_STATUS, "OK", "OTA-OK", bootCount, wakeUpCycleCounter, firstBootTime, 0, 0, 0, 0, 0, "", "", 0);      
		send_data("OK", "OTA OK", startTime, first_boot, "0", true);
        vTaskDelay(pdMS_TO_TICKS(5000));
        esp_restart();
    }
	forceOTA = false;
