...
#else
    ESP_LOGI(TAG, "Inicializace komunikace: LORA");
...
esp_err_t comm_send(comm_msg_type_t type, comm_data_t *data) {
	ESP_LOGI(TAG, "Comm Send: %d, Status: %s", type, data->state);
...
uint8_t map_state_to_code(const char* state) {
    ...
    if (strcmp(state, "FAILED") == 0) return 99;
    return 255; // Neznámý
}

uint8_t map_reason_to_code(const char* reason) {
    if (reason == NULL) return 0;
    if (strcmp(reason, "OK") == 0) return 0;
    if (strcmp(reason, "SLEEP") == 0) return 1;
    if (strcmp(reason, "FAILED") == 0) return 99;
    return 255; // Neznámý
}
          ...
          *out_size = offsetof(lora_universal_packet_t, payload) + (*items_sent * sizeof(lora_queue_item_t));
      	...
      	case MSG_TYPE_STATUS:
            ...
            *items_sent = 1;
            ...
            ESP_LOGI(TAG, "Converted statecode: %d", dst->state_code);
            //commdata->reason;
            ...
            ESP_LOGI(TAG, "Odeslani STATUS do mqtt obis_96_1_1: %s", commdata->meter_data.obis_96_1_1);
            uint64_t sn64 = strtoull(commdata->serialno, NULL, 10);
            // Do sn1 uložíme spodních 32 bitů (maskování hodnotou 0xFFFFFFFF)
		    dst->serialno.sn1 = (uint32_t)(sn64 & 0xFFFFFFFF);
    		// Do sn2 posuneme horní bity o 32 míst doprava
    		dst->serialno.sn2 = (uint8_t)(sn64 >> 32);

            // Řetězce start_time_str a next_wakeup_str NEPOSÍLÁME!
            // ESPHome si je na straně serv eru případně vypočítá z času a wait_time.
...
/*
Pro dosazení 10místného čísla 5542684578 do vaší 5bajtové struktury musíte hodnotu rozdělit pomocí bitových posunů. Číslo 5542684578 (v šestnáctkové soustavě 0x14A5EA7A2) se totiž nevejde do klasického 32bitového registru uint32_t, který má maximum 4294967295.
Protože je cílem minimální délka pro přenos přes LoRa, je toto čistě binární rozdělení (využití všech 5 bajtů jako jednoho velkého 40bitového čísla) tím nejefektivnějším možným řešením.
## Příklad v C / C++
Pro uložení takto velkého čísla v kódu musíte nejprve použít 64bitovou proměnnou typu uint64_t a následně ji "rozřezat" na spodní 4 bajty a horní 1 bajt.

#include <iostream>#include <cstdint>
#pragma pack(push, 1)typedef struct {
    uint32_t sn1; // Spodní 4 bajty (0 až 31. bit)
    uint8_t sn2;  // Horní 1 bajt (32. až 39. bit)
} serialno_t;#pragma pack(pop)
int main() {
    // 10místné sériové číslo (vyžaduje uint64_t)
    uint64_t obis_96_1_1 = 5542684578ULL; 

    serialno_t serialno;

    // Do sn1 uložíme spodních 32 bitů (maskování hodnotou 0xFFFFFFFF)
    serialno.sn1 = (uint32_t)(obis_96_1_1 & 0xFFFFFFFF);

    // Do sn2 posuneme horní bity o 32 míst doprava
    serialno.sn2 = (uint8_t)(obis_96_1_1 >> 32);

    // --- Ověření výsledku ---
    std::cout << "Zapsáno do struktury:" << std::endl;
    std::cout << "serialno.sn1: " << serialno.sn1 << " (0x" << std::hex << serialno.sn1 << ")" << std::endl;
    std::cout << "serialno.sn2: " << (int)serialno.sn2 << " (0x" << std::hex << (int)serialno.sn2 << ")" << std::endl;

    // Zpětná rekonstrukce čísla (např. na straně serveru / brány)
    uint64_t rekonstrukce = ((uint64_t)serialno.sn2 << 32) | serialno.sn1;
    std::cout << std::dec << "Původní číslo po rekonstrukci: " << rekonstrukce << std::endl;

    return 0;
}

## Jak to funguje v paměti?
Číslo 5542684578 je binárně:
00000001 01001010 01011110 10100111 10100010

* serialno.sn1 dostane hodnotu 1247717282 (0x4A5EA7A2), což jsou spodní 4 bajty.
* serialno.sn2 dostane hodnotu 1 (0x01), což je zbývající horní 1 bajt.

Při odesílání struktury přes LoRa (např. pomocí funkce pro odeslání pole bajtů) odešlete přesně 5 bajtů bez jakékoliv textové režie, což je pro LoRaWAN protokoly ideální payload.
Pokud budete data na straně serveru (např. v ChirpStacku nebo Node-RED) dekódovat, dejte mi vědět, v jakém jazyce běží váš parser (např. JavaScript / Payload Formatter). Mohu vám pro něj připravit dekódovací funkci.

obis = 5542684578# Hexadecimal representationhex_val = hex(obis)
print(f"Hex: {hex_val}")# Total bytes neededimport mathbytes_needed = math.ceil(obis.bit_length() / 8)
print(f"Bytes: {bytes_needed}")
# Split into 4 bytes (lower) and 1 byte (upper)# obis = sn2 * 2^32 + sn1sn2 = obis >> 32sn1 = obis & 0xFFFFFFFF
print(f"sn2: {sn2}, sn1: {sn1}")
print(f"Verify: {(sn2 << 32) + sn1}")
*/
