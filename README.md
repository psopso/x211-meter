...
\* \*\*Podpora Energy Dashboardu\*\*: Senzory spotřeby (OBIS kódy 1.8.x a 2.8.x) jsou plně kompatibilní s energetickým panelem Home Assistant (třídy `total\_increasing`).
...
1\. Zkopírujte složku `xt211` do adresáře `custom\_components` ve vaší instalaci Home Assistant.
...
\* \*\*Zpracování dat\*\*: Data jsou do InfluxDB odesílána do measurementu `home\_energy` s tagem `meter`.
